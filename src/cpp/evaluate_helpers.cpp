...
void SparseBayes::evaluate_reestimates(ModelState &state, arma::vec &DeltaML,
                                       arma::ivec &action_vec) {
  arma::vec used_factor = state.relevance_factor.elem(state.used_basis_idx);
  arma::uvec i_pos = arma::find(used_factor > ZERO_FACTOR);
  if (i_pos.n_elem > 0) {
    ...
    action_vec.elem(reest_idx).fill(static_cast<int>(Action::Reestimate));
  }
}

// Evaluate deletion candidates and set action_vec entries for deletions.
void SparseBayes::evaluate_deletions(ModelState &state, arma::vec &DeltaML,
                                     arma::ivec &action_vec) {
  arma::vec used_factor = state.relevance_factor.elem(state.used_basis_idx);
  arma::uvec i_neg = arma::find(used_factor <= ZERO_FACTOR);
  if (i_neg.n_elem > 0 && state.used_basis_idx.n_elem > 1) {
    ...
    action_vec.elem(del_idx).fill(static_cast<int>(Action::Delete));
  }
}

// Evaluate additions: set DeltaML and action_vec for candidate additions.
void SparseBayes::evaluate_additions(ModelState &state, arma::vec &DeltaML,
                                     arma::ivec &action_vec) {
  arma::uvec add_mask =
      arma::conv_to<arma::uvec>::from(state.relevance_factor > ZERO_FACTOR);
  ...
  if (add_idx.n_elem > 0) {
    ...
    action_vec.elem(add_idx).fill(static_cast<int>(Action::Add));
  }
}
