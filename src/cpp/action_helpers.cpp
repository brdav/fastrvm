...
void SparseBayes::ReestimateBasis(ModelState &state, arma::uword j,
                                  double new_alpha) {
...
void SparseBayes::AddBasisFunction(ModelState &state, arma::uword nu,
  ...
  arma::mat Sigma = state.Sigma - s_i * tmp.t();  // MxM
  ...
  state.Sigma.submat(0, 0, M - 1, M - 1) = Sigma;
...
void SparseBayes::DeleteBasisFunction(ModelState &state, arma::uword j) {
...
void SparseBayes::PerformAction(ModelState &state, Action action, arma::uword j,
                                arma::uword nu, double new_alpha,
                                const arma::mat &basis,
                                const arma::vec &Phi_vec, int &add_count,
                                int &delete_count, int &update_count) {
  if (action == Action::kReestimate) {
    ReestimateBasis(state, j, new_alpha);
    update_count++;
  } else if (action == Action::kAdd) {
    AddBasisFunction(state, nu, new_alpha, basis, Phi_vec);
    add_count++;
  } else if (action == Action::kDelete) {
    DeleteBasisFunction(state, j);
    delete_count++;
  }
}
